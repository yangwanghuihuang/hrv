<template>
  <div>
    <Modal v-model="modal1" title="出账任务" width="560" height="400" top="20px" @on-cancel="cancel">
      <div slot="header">
        <Button type="default" @click="confirmTask()">确定</Button>
      </div>
      <div style="text-align:left">
        <div class="contentSettle">
          <span class="text">第{{pageIndex}}页 / 共{{totalPage}}页 共{{dataCount}}条</span>
          <Page
            class="pageStyle"
            show-total
            simple
...
export default {
  ...
  mounted () {
    this.tableHeight = (document.documentElement.clientHeight * (5 / 10))
    this.$http
